...
def nomalize_data(data, url, tenant, branch):
    ...
    vulnerabilities = []
    for report_result in data['report'].get('results', []):
        ...
        if branches != []:
            for branch in branches:
                ...
                if response.returncode == 0:
                    ...
                    print(folder_name)
                    bandit_scan_command = r"files\bandit.exe  %s -r -f json --silent" %folder_name
                    result = subprocess.run(bandit_scan_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
