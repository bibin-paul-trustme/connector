...
def count_calculation(data, url, tenant, branch, chunk_name):
    ...
    base_url = config['LOCAL']['trustme_phpmd_count_upload']
        ...
        if branches != []:
            for branch in branches:
                ...
                if response.returncode == 0:
                    ...
                    phpmd_scan_command = r"phpmd %s json codesize,naming" %folder_name
                    print(phpmd_scan_command)
                    result = subprocess.run(phpmd_scan_command, shell=True, capture_output=True, text=True)
